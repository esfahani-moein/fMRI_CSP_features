...
%% CSP Transform Matrix
idx = 1:1:10;
results = cell(3,numel(idx));

for i =1:numel(idx)
    nbFilters = idx(i);  % output features are 2x
    CSP_data = filterCSP(dataClass1,dataClass2,nbFilters);
    %% Feature Extraction

    Features1 = zeros(length(Label),size(CSP_data.CSP_Matrix,1));
    Features2 = zeros(length(Label),size(CSP_data.CSP_Matrix,1));
    Features_raw = zeros(size(CSP_data.CSP_Matrix,1),CSP_data.nTimePoints, length(Label));

    for k=1:length(Label)
        x1 = Dataset(:,:,k);
        xx = CSP_data.CSP_Matrix*x1;
    ...
    results{3,i} = CSP_data;
end

%% Save Results
file_path = fullfile('results', 'results_CSP.mat');
save(file_path,"results")
file_path = fullfile('results', 'labels.mat');
save(file_path, "Label")